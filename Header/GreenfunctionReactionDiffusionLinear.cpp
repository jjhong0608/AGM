...
AGM::GreenfunctionReactionDiffusionLinear::GreenfunctionReactionDiffusionLinear(double tm, double tau, double tp,
                                                                                double mpl, double mpr, double c)
        : GreenfunctionReactionDiffusion(tm, tau, tp, mpl, mpr, c) {
    GreenfunctionReactionDiffusion::alpha = std::sqrt(c / mpl);
}

double AGM::GreenfunctionReactionDiffusionLinear::green_integral(char pos) const {
...
double AGM::GreenfunctionReactionDiffusionLinear::green_integral_t(char pos) const {
    double cl = isclose(tau, tm) ? ZEROVALUE : UNITVALUE / (tau - tm);
    double cr = isclose(tau, tp) ? ZEROVALUE : UNITVALUE / (tau - tp);
...
double AGM::GreenfunctionReactionDiffusionLinear::green_integral_tau(char pos) const {
    double cl = isclose(tau, tm) ? ZEROVALUE : UNITVALUE / (tau - tm);
    double cr = isclose(tau, tp) ? ZEROVALUE : UNITVALUE / (tau - tp);
...
double AGM::GreenfunctionReactionDiffusionLinear::green_integral_ttau(char pos) const {
    double cl = isclose(tau, tm) ? ZEROVALUE : UNITVALUE / (tau - tm);
    double cr = isclose(tau, tp) ? ZEROVALUE : UNITVALUE / (tau - tp);
